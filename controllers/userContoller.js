    ...
    try {
        ...
        if (expiresAt < Date.now()) {
            ...
            if (isMatch) {
                ...
                const message = "Verification failed or link OTP is invalid";
                res.status(404).json({ message });
              }
        }
        ...
        console.log(error);
        res.status(404).json({ message: "Ada yang salah "})
    }
...
export const followWritter = async(req, res, next ) => {
    try {
        ...
        if (checks) return res.status(201).json({
            success: false,
            message: "Kamu telah mengikuti writer"
        });
